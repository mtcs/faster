...
faster::workerFddBase * faster::newWorkerSDL(unsigned long int id, fddType type, size_t size){
	switch (type){
		case String: return new   _workerIFdd<char, std::string>(id, type, size); break;
		case CharV: return new    _workerIFdd<char, std::vector<char>>(id, type, size); break;
		case IntV: return new     _workerIFdd<char, std::vector<int>>(id, type, size); break;
		case LongIntV: return new _workerIFdd<char, std::vector<long int>>(id, type, size); break;
		case FloatV: return new   _workerIFdd<char, std::vector<float>>(id, type, size); break;
		case DoubleV: return new  _workerIFdd<char, std::vector<double>>(id, type, size); break;
	}
	return NULL;
}
