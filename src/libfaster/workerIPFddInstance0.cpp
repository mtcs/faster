...
faster::workerFddBase * faster::newWorkerSDL(unsigned long int id, fddType type, size_t size){
	switch (type){
		case CharP: return new    _workerIFdd<char, char*>(id, Char, type, size); break;
		case IntP: return new     _workerIFdd<char, int*>(id, Char, type, size); break;
		case LongIntP: return new _workerIFdd<char, long int*>(id, Char, type, size); break;
		case FloatP: return new   _workerIFdd<char, float*>(id, Char, type, size); break;
		case DoubleP: return new  _workerIFdd<char, double*>(id, Char, type, size); break;
	}
	return NULL;
}
