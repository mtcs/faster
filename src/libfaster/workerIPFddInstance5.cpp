...
faster::workerFddBase * faster::newWorkerSDL(unsigned long int id, fddType type, size_t size){
	switch (type){
		case CharP: return new    _workerIFdd<std::string, char*>(id, type, size); break;
		case IntP: return new     _workerIFdd<std::string, int*>(id, type, size); break;
		case LongIntP: return new _workerIFdd<std::string, long int*>(id, type, size); break;
		case FloatP: return new   _workerIFdd<std::string, float*>(id, type, size); break;
		case DoubleP: return new  _workerIFdd<std::string, double*>(id, type, size); break;
	}
	return NULL;
}
