...
faster::workerFddBase * faster::newWorkerSDL(unsigned long int id, fddType type, size_t size){
	switch (type){
		case Char: return new    _workerIFdd<char, char>(id, Char, type, size); break;
		case Int: return new     _workerIFdd<char, int>(id, Char, type, size); break;
		case LongInt: return new _workerIFdd<char, long int>(id, Char, type, size); break;
		case Float: return new   _workerIFdd<char, float>(id, Char, type, size); break;
		case Double: return new  _workerIFdd<char, double>(id, Char, type, size); break;
	}
	return NULL;
}
