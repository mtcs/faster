...
faster::workerFddBase * faster::newWorkerSDL(unsigned long int id, fddType type, size_t size){
	switch (type){
		case Char: return new    _workerIFdd<double, char>(id, type, size); break;
		case Int: return new     _workerIFdd<double, int>(id, type, size); break;
		case LongInt: return new _workerIFdd<double, long int>(id, type, size); break;
		case Float: return new   _workerIFdd<double, float>(id, type, size); break;
		case Double: return new  _workerIFdd<double, double>(id, type, size); break;
	}
	return NULL;
}
