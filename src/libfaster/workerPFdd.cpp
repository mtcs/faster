...
void workerFdd<T *>::map (workerFdd<U> & dest, PmapPFunctionP<T,U> mapFunc){
	...
	for (int i = 0; i < localData.getSize(); ++i){
		std::pair<U,size_t> r = mapFunc(localData[i], ls[i]);
		dest[i] = r.first; 
		dls[i] = r.second;
	}
	//std::cerr << "END ";
...
void workerFdd<T *>::map (workerIFdd<L,U> & dest, ImapPFunctionP<T,L,U> mapFunc){
	...
	#pragma omp parallel for 
	for (int i = 0; i < s; ++i){
		std::pair<L,U> r = mapFunc(localData[i], ls[i]);
		ok[i] = r.first;
		dest[i] = r.second;
...
void workerFdd<T *>::map (workerIFdd<L,U> & dest, IPmapPFunctionP<T,L,U> mapFunc){
	...
	#pragma omp parallel for 
	for (int i = 0; i < localData.getSize(); ++i){
		std::tuple<L,U,size_t> r = mapFunc(localData[i], ls[i]);
		ok[i] = std::get<0>(r);
		dest[i] = std::get<1>(r);
		dls[i] = std::get<2>(r);
	}
...
T * workerFdd<T *>::reduce (size_t & rSize, PreducePFunctionP<T> reduceFunc){
	...
	#pragma omp parallel 
	{
		...
		std::pair<T *, size_t>  partResult(localData[tN], ls[tN] );
		T * a, * b;
		size_t aSize, bSize;
